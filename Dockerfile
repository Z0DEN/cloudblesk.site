...
ENV DJANGO_PROJECT /home/main/cloudblesk.site/myproject
ENV NGINX_FILES /home/main/nginx_files/
...
COPY $NGINX_FILES /nginx_files/ 
COPY $DJANGO_PROJECT /myproject/

RUN mv /nginx_files/conf/nginx.conf /etc/nginx/nginx.conf
...
RUN pip3 install -r /nginx_files/python/req.txt

EXPOSE 80 443

STOPSIGNAL SIGTERM

CMD ["/usr/bin/supervisord", "-c", "/nginx_files/conf/supervisor.conf"]
